...
def _runtime_extension_scripts(text: str) -> set[str]:
    return set(
        re.findall(r"codex-seo run --extension banana ([A-Za-z0-9_]+\.py)", text)
    )
...
def test_standalone_installer_copies_scripts_beside_skill_file():
    ...
    assert 'cp "${ASSET_SOURCE}/scripts/"*.py "${SKILL_DIR}/scripts/"' in installer
    assert '"$HOME/.codex/skills/seo/bin/codex-seo" run' in installer


def test_top_level_installer_keeps_extension_scripts_under_core_extension_tree():
    installer = _text(ROOT / "install.sh")
    assert 'for dir_name in bin scripts schema pdf hooks extensions; do' in installer
    assert 'cp -r "${TEMP_DIR}/codex-seo/${dir_name}/." "${SKILL_DIR}/${dir_name}/"' in installer
