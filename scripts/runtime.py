...
ALLOWED_CORE_SCRIPTS = frozenset(
    {
        ...
        "portability_check.py", "consistency_check.py", "run_skill_workflow.py",
        "run_api_smoke_suite.py",
...
def _data_dir(root: Path) -> tuple[Path, str]:
    ...
    plugin_data = os.environ.get("CODEX_PLUGIN_DATA")
    if plugin_data:
...
def _status(root: Path) -> dict[str, Any]:
    ...
    try:
        managed_browser = browser_dir.is_dir() and any(
            child.name.startswith(("chromium-", "chromium_headless_shell-"))
            for child in browser_dir.iterdir()
        )
    except OSError:
        managed_browser = False
    # The legacy Codex installer lets Playwright use its platform cache, while
    # ``codex-seo setup`` stores Chromium under the managed data directory.
    browser_installed = managed_browser or bool(state.get("browser_ready"))
    return {
        "ready": not reasons,
        "mode": mode,
