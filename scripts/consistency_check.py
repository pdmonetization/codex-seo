...
ORPHAN_SCRIPT_WHITELIST = {
    ...
    "generate_premium_audit_report.py", "probe_skill_selection_provider.py",
    "runtime.py", "seo_pipeline_utils.py",
}
...
def check_skill_dir_script_refs(files, texts):
    """Validate scripts invoked through Codex's portable skill-root variable.
    ...
    """
    pat = re.compile(r'\$\{CODEX_SKILL_DIR\}/scripts/([A-Za-z0-9_]+\.py)\b')
    errors = []
    for f in texts:
        for script in sorted(set(pat.findall(read(f)))):
            ...
            if ext_root:
                ...
                errors.append(f"{f}: CODEX_SKILL_DIR script used outside a skill layout")
                continue
...
def check_agent_refs(files, texts):
    agents = {os.path.basename(f)[:-5] for f in files
              if f.startswith("agents/") and f.endswith(".toml")}
    pat = re.compile(r'agents/([a-z0-9-]+)\.(?:md|toml)')
    errors = []
    for f in texts:
        ...
        for m in sorted(set(pat.findall(read(f)))):
            if m not in agents and m not in DOC_EXAMPLE_AGENTS:
                errors.append(f"{f}: dead Codex agent ref agents/{m}.toml")
    return errors
