...
Read `.seo-cache/site-meta.json`, relevant page analysis, and existing content
plans when available. Write the completed brief to
`.seo-cache/content-briefs/{topic-slug}.json`. Treat corrupt cache entries as
missing and ignore the cache when the user requests a refresh.
