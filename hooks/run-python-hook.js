...
function pythonCandidates() {
  const candidates = [];
  if (process.env.CODEX_SEO_PYTHON) {
    candidates.push({
      label: "CODEX_SEO_PYTHON",
      exe: stripWrappingQuotes(process.env.CODEX_SEO_PYTHON),
      args: [],
    });
  }
  candidates.push(
...
function main() {
  ...
  console.error(
    "Codex SEO hook could not find Python. Tried CODEX_SEO_PYTHON, py -3, python3, python.",
  );
  process.exit(1);
}
